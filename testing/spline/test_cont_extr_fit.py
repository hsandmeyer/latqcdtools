...
knots, res, res_err, chi_dof = multi_spline_fit(linear, 2, [5, 6, 7], xdata, ydata,
        order = order, ncoeffs = None, knots = [5.3, 5.5, 5.6],
        tol = 1e-8, nsteady_deriv = 2, algorithms = ['curve_fit'],
        constraints = constraints, always_return = True)


plot_dots(xdata[0], ydata[0])
plot_dots(xdata[1], ydata[1])
plot_dots(xdata[2], ydata[2])

coeffs = [linear(5, *i) for i in res]
plot_func(constraint_spline, args = (knots, coeffs, order, 2, 0, constraints))
