...
def diff_hess(params, func, args=(), eps=None, expand=False):
    if expand:
        def wrap_func(params): return func(*(tuple(params) + tuple(args)))
    ...
    for i in range(len(params)):
        ...
        downdown[i] = down[i]
        downup[i] = down[i]
        ret[i][i] = (func(up, *args)+func(down, *args)-2 *
                     func(params, *args))/(4*(epsi/2.0)**2)
...
def diff_fit_grad(x, params, func, args=(), eps=None, expand=False):
    ...
    if expand:
        def f(p): return func(x, *(tuple(p) + tuple(args)))
    else:
        def f(p): return func(x, p, *args)
...
# Implementation for usage with algopy which can handle numpy operations but is very
# slow and buggy
...
def alg_hess(x, func, args=()):
    if not algopy_avail:
        raise ImportError(
            "Algopy not installed. Please install algopy or use diff_hess")
