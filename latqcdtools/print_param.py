...
# get the string of a number + error 1.234567+-0.324456 -> 12.34(33) (numb_err_dig = 2)


def get_err_str(param, param_err, numb_err_dig=1):
    if (numb_err_dig < 1):
        print("Number of error digits has to be larger than 0!")
        exit(-1)
    if param < 0:
        param = -param
        sign = -1
    ...
    # index for rounding the error and the paramter
    ...
    if (numdig < 0):
        numdig = 0
    # floor does not support a second index -> we have to multipy and then devide
    ...
    # if the error is larger or equal 1
    if (relnum >= 0):
...
def get_err_str_auto_tex(param, param_err, numb_err_dig=1, mulicon="x"):
