...
class CorrFitterRatio:
    ...
    def _swap(self, res, res_err):
        if (abs(res[1]) > abs(res[2]) and abs(res[2]) > 1e-5
                or (abs(res[1]) < 1e-5 and abs(res[2]) > 1e-5)):
    ...
    def corr_fit(self, xmin=-np.inf, xmax=np.inf, start_params=None,
        ...
        if start_params is None:
            try:
                ...
                if self._nstates == 2:
                    start_params = [tmp_params[0]/tmp_params[2],
                                    tmp_params[1], tmp_params[3]]
                else:
        ...
        print_res("Final starting parameters for ratio fit",
        ...
        print_res("Fit result from ratio fit", res,
                  res_err, chi_dof, level="INFO")
...
def hess_one_state_ratio(x, m, Nt):
    ...
    return np.array([[0.25 * 1/np.sinh(m * (1 - Nt/2 + x)) *
                      ((Nt - 2 * x)**2 * np.cosh(0.5 * m * (Nt - 2 * x))
                       - (Nt - 2 * (1 + x))**2 *
                          np.cosh(0.5 * m * (Nt - 2 * x))
                       * 1/np.sinh(m * (1 - Nt/2 + x))**2 - 2 * (Nt - 2 * x) * (Nt - 2 * (1 + x)) *
                       np.sinh(0.5 * m * (Nt - 2 * x)) *
                       np.tanh(0.5 * m * (Nt - 2 * (1 + x)))
                       + (Nt - 2 * (1 + x))**2 *
                          np.cosh(0.5 * m * (Nt - 2 * x))
...
def hess_two_state_ratio(x, A2, m2, m1, Nt):
    ...
    res = np.array([
        [
            ...
            + (2 * A2 * ntx1 * m2c_1 * (m1c + A2*m2c)
               * m2s_1)/(m1c_1 + A2*m2c_1)**3
            ...
            + (2 * A2**2 * ntx1**2 * (m1c + A2*m2c)
               * m2s_1**2)/(m1c_1 + A2*m2c_1)**3
            - (2 * A2**2 * ntx1 * ntx * m2s_1 * m2s)/(m1c_1 + A2*m2c_1)**2,
            (2 * A2 * ntx1**2 * (m1c + A2*m2c) *
             m1s_1 * m2s_1)/(m1c_1 + A2*m2c_1)**3
