#!/usr/bin/env python3
import traceback
from latqcdtools.corr_fitting import *
from latqcdtools.corr_ratio_fitting import *
from latqcdtools.readin import *
import latqcdtools.jackknife as jk
from latqcdtools.write import *
import latqcdtools.bootstr as bs
from latqcdtools.effmass import *
from latqcdtools.remove_osc import *
...
class PerformCorrFit:
    ...
    default_args = {
        ...
        # Fit the ration G(nt+1)/G(nt) instead of G(nt)
        'ratio_fit': False,
        # Use quantiles for error distribution (non-gaussian -> ng)
        'ng_btstr': True,
      ...
      # If we do not have a fit flag switch to default direct fit
        ...
        if self._ratio_fit and any([self._jack_data, self._btstr_data, self._direct_data,
            ...
            raise ParameterError(
                "Ratio fit can only be combined with ratio_data")

        if self._ratio_fit and self._nstates_osc > 0:
            raise ParameterError(
                "Ratio fit does only work for --nstates-osc 0 ")
        ...
        if self._correlated and self._scnd_btstr:
            raise ParameterError(
                "Correlated fits not possible with second bootstrap")

        if self._seed and self._scnd_btstr:
            raise ParameterError(
                "Using a fixed seed and a second bootstrap is not supported")

        if self._min_cov_det and self._ratio_fit:
            raise ParameterError(
                "Ratio fit and MinCovDet is not supported at the same time")

        # if self._min_cov_det and (self._btstr_data or self._jack_data or self._btstr_fit or
        #        self._btstr_data):
        #    raise ParameterError("MinCovDet and bootstrap or jackknife does not make sense")

        if self._priorsigma is None and self._priorval is not None:
            raise ValueError(
                "--prior-sigma has to be passed along with --prior-val")

        if self._priorsigma is not None and self._priorval is None:
            raise ValueError(
                "--prior-val has to be passed along with --prior-sigma")
    ...
    def _calc_fit_interval(self):
        ...
        if self._fit_interval is None:
            ...
            # Feel free to change
            self._fit_interval = [self._nt_min_start, min(
                self.xmax(0), int(self._Nt / 2)) - stopnumb + 1]
        else:
    ...
    def check_data_consistency(self):
        ...
        for i in self._data:
            if len(i) != last_len:
                raise ValueError(
                    "Number of data points per x value inconsistent!")
    ...
    def read_in_jk(self):
        ...
        self._ydata, self._edata = jk.jackknife(
    ...
    def read_in_jk_ratio(self):
        self.read_in_jk()
        self._ratio_ydata, self._ratio_edata = jk.jackknife(corr_ratio,
    ...
    def read_in_std_err_ratio(self):
        ...
        self._ratio_ydata, self._ratio_edata = mean_and_err(
            self._rat_data, axis=1)
        self._ratio_xdata = self._xdata[:-1]
        if self._correlated:
            self._ratio_cov = self._calc_ratio_cov()
    ...
    def _compute_meff(self):
        ...
        if self._jack_data or self._std_err_data or self._ratio_data:
            if self._nstates_osc > 0:
                rm_osc, rm_osc_err = jk.jackknife(
                    remove_osc, self._data, self._numb_blocks, args=(self._xdata,))

            else:
                eff_mass, eff_mass_err = jk.jackknife(calc_eff_mass, self._data,
        ...
        if self._btstr_data:
            ...
            if self._nstates_osc > 0:
                rm_osc, rm_osc_err = bs.bootstr(remove_osc, self._data, self._numb_samples,
                                                err_by_dist=self._ng_btstr, args=(
                                                    self._xdata,),
                                                seed=self._seed)
            else:
        ...
        if self._sample_data:
            if self._nstates_osc > 0:
                ...
                for i in range(len(self._data[0])):
                    results.append(remove_osc_av(
                        self._data[:, i], self._xdata))
    ...
    def _fit_corr_fr_data(self, data, xmin, xmax, start_params):
        ...
        if self._scnd_btstr:
            ...
            if self._correlated:
                ...
                corr_fitter = CorrFitter(
                    self._xdata, ydata, edata, Nt=self._Nt)
    ...
    def jk_fit(self, xmin, xmax, start_params):
        ...
        ((res, tmp_err, chi_dof, aicc, pcov),
         (res_err, tmp_err_err, chi_err, aicc_err, pcov_err)) = jk.jackknife(
    ...
    def _get_start_params_array(self, xmin, xmax):
        ...
        if self._start_params is None:
            ...
            start_params = self._get_start_params(
                self._res, self._res_err, ratio=ratio)
            ...
            if not (self._osc_corr_fitter is None
                ...
                if self._try_all or len(start_params_array) == 0:
                    if start_params is not None:
                        start_params_array.append(
                            (start_params, "separate fits"))
