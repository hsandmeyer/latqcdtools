...
from latqcdtools.spline import *
...
edata:
    error of ydata
xmin:
    xmin of data used for spline calculation
xmax:
    xmax of data used for spline calculation
sp_xmin:
    xmin for output data
sp_xmax:
    xmax for output data
xpline:
    instead of linspace between sp_xmin and sp_xmax use these x values for spline output
ncoeffs:
    number of coeffecients. If None best choice is tuned automatically
order:
    order of the spline
npoints:
    number of output points if not provided xpline
s:
    smoothing factor for automatic choice of ncoeffs
"""
...
def bspline(xdata, ydata, edata=None, xmin=None, xmax=None, sp_xmin=None,
    ...
    shared_lib_path = os.path.dirname(os.path.realpath(
        __file__)) + "/../bspline_build/libbspline_py.so"
    ...
    if ncoeffs is None:
        for nc in range(order + 1, len(xdata)):
            _, res_y, _ = bspline(xdata, ydata, edata,
                                  ncoeffs=nc, order=order, xspline=xdata)
            chi_dof = np.sum((res_y - ydata)**2 / edata**2) / \
                (len(xdata) - order)
            if chi_dof < s:
                ncoeffs = nc
                print("Bspline: Choose ncoeff = ",
                      nc, "with chi/d.o.f =", chi_dof)
                break

        if ncoeffs is None:
            ncoeffs = len(xdata)
            print("Bspline: Could not fullfill chi/d.o.f <", s, ".")
            print("Choose ncoeff = length(xdata) = ",
                  ncoeffs, "with chi/d.o.f =", chi_dof)

    bspline_interpolate(ctypes.c_void_p(xdata.ctypes.data), ctypes.c_void_p(ydata.ctypes.data),
                        ctypes.c_void_p(edata.ctypes.data),
                        ctypes.c_int(len(xdata)), ctypes.c_void_p(
                            xout.ctypes.data),
                        ctypes.c_void_p(yout.ctypes.data), ctypes.c_void_p(
                            yout_err.ctypes.data),
                        ctypes.c_int(len(xout)), ctypes.c_int(order), ctypes.c_int(ncoeffs))
...
def multi_spline_fit(fit_func, nparams, add_params, xdata, ydata, edata=None, knots=None,
    ...
    if knots is None:
        ...
        if randomization_factor > 0:
            knots = random_knots(
                xdata, nknots, constraints, randomization_factor)
        else:
    ...
    if start_params is None and (len(constraints) > 0):
        lg.progress("multi_spline_fit: No startparameters available,"
        ...
        remove_inds = spline_ind_sel(
            None, constraints, knots, order, nsteady_deriv)
        start_params = np.delete(start_params, remove_inds, axis=0)
        start_params = np.concatenate(start_params)

    if ncoeffs is None:
        ncoeffs = get_num_spline_coeffs(
            order, nsteady_deriv, nknots, constraints)
...
def random_knots(xdata, nknots, constraints, randomization_factor=1):
    ...
    sample_xdata = np.random.choice(flat_xdata,
                                    int(nknots + 1 + (1 - randomization_factor)
                                        * (len(flat_xdata) - nknots)),
                                    replace=False)
