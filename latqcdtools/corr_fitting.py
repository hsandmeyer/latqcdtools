...
class CorrFitter(Fitter):
    ...
    #Weight that are needed for computing smaller fit intervals in parameter estimation


    # If edata contains errors, they are not squared: edata=sqrt(variance)
    # If it is the covariance matrix they are interpreted as squared:
    # std_err = sqrt(diag(edata))
    ...
    def _swap(self, i, res, res_err):
        if ((abs(res[i + 2]) > abs(res[i + 0]) and abs(res[i + 3]) > 1e-5
                or (abs(res[i + 1]) < 1e-5 and abs(res[i+3]) > 1e-5)
                or (res[i + 0] < 0 and res[i + 2] > 0))
                and not (res[i + 0] > 0 and res[i + 2] < 0)):
