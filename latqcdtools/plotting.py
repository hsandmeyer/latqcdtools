...
"""Collection of handy wrappers for plotting different data sets using the matplotlib"""


zod = 1  # use for zorder in plot commands will be increased
colors_1 = ['#d32d11', '#0081bf', '#e5af11', '#7c966d',
            '#7570b3', '#ff934f', '#666666', '#D186B3']
colors_2 = ['#396AB1', '#DA7C30', '#3E9651', '#CC2529',
            '#535154', '#6B4C9A', '#922428', '#948B3D']
colors_3 = ['#A6CEE3', '#1F78B4', '#B2DF8A', '#33A02C',
...
default_params = {
    ...
    # Minimum xvalue to be plotted
    'xmin': None,

    # Maximium xvalue to be plotted
    'xmax': None,
    ...
    # If labelsintplot is true use this to shift the positions of the ylabels
    'xlabelpos': None,
...
def set_params(ax=plt, **params):
    ...
    **params :
    ...
    if params['show_leg']:
        ...
        leg.set_zorder(zod)
        if (params['legend_linewidth'] != None):
    ...
    if func is not None:
        if yedata is not None:
            if xedata is not None:
                xdata, ydata, yedata, xedata = func(
                    xdata, ydata, yedata, xedata, *func_args)
            else:
...
def plot_bar(xdata, ydata, **params):
    ...
    if params['color'] is not None:
        bar = ax.bar(xdata, ydata, color=params['color'], zorder=zod,
                     linewidth=params['linewidth'], alpha=params['alpha'], **optional)
    else:
        bar = ax.bar(xdata, ydata, zorder=zod,
                     linewidth=params['linewidth'], alpha=params['alpha'], **optional)
...
def plot_hist(data, logx=False, bins=None):
    if logx:
        #xmin = 10**math.floor(np.log10(np.min(data)))
        #xmax = 10**math.ceil(np.log10(np.max(data)))
        xmin = np.min(data)
        xmax = np.max(data)
...
def plot_lines(xdata, ydata, yedata=None, xedata=None, **params):
    ...
    # line = ax.plot(xdata*xscale, ydata*yscale, color=col, linewidth=params['linewidth'],
...
def save_func(func, filename, args=(), func_err=None, args_err=(), grad=None, func_sup_numpy=False,
    ...
    if func_err is not None:
        ...
        if grad is None:
            lg.warn("#Warning! Used numerical derivative!")
            wrap_grad = None
...
# def latexify(fig_width=16, fig_height=11, font_size=11):
...
def latexify(fig_width=10, fig_height=7, font_size=9, static_margins=False, init=True, init_fig=None,
    ...
    set_markers()
    plt.rcParams['legend.handlelength'] = 1.5
    plt.rcParams['text.latex.preamble'] = r"\usepackage{amsmath}\usepackage{lmodern}\usepackage{amssymb}\usepackage{braket}\usepackage{xcolor}"
...
def plot_cov(cov, filename=None, title=None, notex=False, ignore_first=0, norm=True,
    ...
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    #heatmap = plt.pcolor(ncov[ignore_first:, ignore_first:], cmap=plt.cm.Blues)
    if xmin is None:
        off_x = 0
    else:
        off_x = xmin
    ...
    plt.pcolormesh(
        xrange, yrange, ncov[ignore_first:, ignore_first:], cmap="Blues")
...
def zoom_axis(ax, width, height, zx_min, zx_max, zy_min, zy_max, loc=1, loc1=2, loc2=4, borderpad=0.5):
    ...
    mark_inset(ax, axins, loc1=loc1, loc2=loc2, fc="none",
               ec="0.5", linewidth=0.5, zorder=zod)
    zod += 1
...
def plot_data_zoom(width, height, zx_min, zx_max, xdata, ydata,
    ...
    if zy_max is None:
        ...
        zy_max = np.min(tmp)
        for i in range(len(xdata)):
            if xdata[i] < zx_max and xdata[i] > zx_min:
                if zy_max < tmp[i]:
                    zy_max = tmp[i]
