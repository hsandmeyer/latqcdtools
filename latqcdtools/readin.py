...
def read_in_pure_no_numpy(filename, col1=1, col2=2, symmetrize=False):
    ...
    if symmetrize:
        if max(xdata) != Nt - 1:
            raise ValueError(
                "The number of x values does not correspond to the largest of its values")
        if Nt % 2 != 0:
...
def read_in_pure(filename, col1=1, col2=2, symmetrize=False):
    xdata, data, nconfs = read_in_pure_no_numpy(
        filename, col1, col2, symmetrize)
    return np.array(xdata), np.array(data), nconfs


def read_in_pure_trans(filename, col1=1, col2=2, symmetrize=False):
    xdata, data, _ = read_in_pure(filename, col1, col2, symmetrize)
