...
class Extrapolator:
    ...
    def __init__(self, Nts, xdata, ydata, edata=None, data_input=None,
        ...
        if data_input == 'sample' and method == 'btstr':
            raise ValueError(
                "Bootstrapping on a bootstrap sample is not a good idea")
        ...
        if self._knots is None:
            # Array of number of knots.
            # For each number of knots a separate extrapolation is performed
    ...
    def _auto_plot_range(self):
        ...
        if self._extr_xdata is None:
            self._extr_xdata = np.linspace(
                self._plot_xmin, self._plot_xmax, 1000)
    ...
    def compute_err_band(self, inv_Nt_squared):
        extr_sample, extr_data, extr_data_err = self._compute_err_band(
            inv_Nt_squared)
    ...
    def compute_extrapolation(self, extr_xdata=None):
        ...
        if not self._method == 'direct':
            self._extr_sample, self._extr_data, self._extr_data_err = self._compute_err_band(
                0)
        else:
    ...
    def _compute_samples(self, nknots):
        ...
        if self._method == 'gauss_btstr':
            sample, tmp_res, tmp_res_err = bs.bootstr_from_gauss(
                ...
                args={'edata': self._av_edata,
                      'start_params': res, 'nknots': nknots},
            ...
            with open(self._folder + self._outname + "_coeffs.txt", "w") as fout:
                ...
                for n in range(len(self._res_sample)):
                    ...
                    for i in range(len(self._res_sample[n])):
                        print(
                            i, *self._res_sample[n][i][0].flatten(), end=" / ", file=fout)
    ...
    def read_extrapolation(self, extr_xdata=None):
        ...
        with open(self._plot_results[0]) as fin:
            for line in fin:
                ...
                if append_to_params:
                    self._direct_res[-1].append([float(i)
                                                for i in line.split()])
            ...
            with open(self._plot_results[2]) as fin:
                for line in fin:
                    ...
                    if not line.startswith('#'):
                        coeffs = [float(i)
                                  for i in line.split('/')[0].split()[1:]]
                        coeffs = np.array(coeffs).reshape(-1, 2)
                        knots = np.array([float(i)
...
def read_in_extr_files(files, xdata_col=1, ydata_col=2, edata_col=3, Nts=None,
    ...
    for filename in files:
        if read_method == 'direct':
            ...
            if edata_col is None:
                ...
                tmp_x, tmp_e, nconfs = read_in_pure(
                    filename, xdata_col, edata_col)
                edata.append(tmp_e)
    ...
    if len(Nts) == 0:
        raise ValueError(
            "Could not get Nt from filename. Please pass with --Nts")

    if len(xdata) != len(Nts):
        raise ValueError(
            "Number of filenames does not agree with number of Nts")
