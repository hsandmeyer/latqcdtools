...
def write_eff_mass(out_name, eff_mass, eff_mass_err, xdata, osc=False):
    ...
    for i in range(len(eff_mass)):
        if osc:
            output.write(str(xdata[i]) + " " + str(eff_mass[i]
                                                   ) + " " + str(eff_mass_err[i]) + '\n')
        else:
            output.write(
                str(xdata[i] + 0.5) + " " + str(eff_mass[i]) + " " + str(eff_mass_err[i]) + '\n')
    output.close()
    ...
    with open(out_name, "w") as f:
        ...
        for i in range(len(ranges)):
            for res in sample_data[i]:
                write_line(f, ranges[i], res[0], res[1],
                           res[2], res[3], nstates, nstates_osc)
...
def write_line(f, fit_range, fit_res, fit_err, chi_dof, aicc,
    ...
    for osc in range(nstates_osc):
        osc += nstates
        f.write(str(abs(fit_res[2*osc + 1])) +
                " " + str(fit_err[2*osc + 1]) + " ")
...
def write_corr(out_name, xdata, ydata, ydata_err):
    ...
    for i in range(len(ydata)):
        output.write(str(xdata[i]) + " " + str(ydata[i]
                                               ) + " " + str(ydata_err[i]) + '\n')
    output.close()
